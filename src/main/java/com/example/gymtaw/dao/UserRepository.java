...
public interface UserRepository extends JpaRepository<UserEntity, Integer> {
    ...
    @Query("select u from UserEntity u where (u.idRolEntity.id = 1 or u.idRolEntity.id = 2) and " +
            "u.id not in (select ut.trainer.id from UserHasTrainerEntity ut where ut.userEntity.id = :userId)")
    List<UserEntity> findTrainersNotAssignedToUser(@Param("userId") Integer userId);

    @Query("select u from UserEntity u where u.idRolEntity.id = 4 and " +
     "u.id not  in (select ut.userEntity.id from UserHasTrainerEntity ut where ut.trainer.id = :trainerId)")
    List<UserEntity> findClientsNotAssignedToTrainer(@Param("trainerId") int trainerId);

    @Query("select u from UserEntity u where (u.idRolEntity.id = 1 or u.idRolEntity.id = 2) and " +
            "u.id in (select ut.trainer.id from UserHasTrainerEntity ut where ut.userEntity.id = :userId)")
    List<UserEntity> findTrainersAssignedToUser(@Param("userId") int userId);

    @Query("select u from UserEntity u where u.idRolEntity.id = 4 and " +
            "u.id in (select ut.userEntity.id from UserHasTrainerEntity ut where ut.trainer.id = :trainerId)")
