...
public interface ExerciseRepository extends JpaRepository<ExerciseEntity, Integer> {
    ...
    @Query("select e from ExerciseEntity e join ValoracionEntity v on v.exercise.id = e.id where v.user.id = :idCliente and v.trainer.id = :idEntrenador order by e.id")
    public List<ExerciseEntity> getExerciseEntitiesByIdClienteIdEntrenadorAndHaveReview(Integer idCliente, Integer idEntrenador);
